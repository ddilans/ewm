...
static void init_geometry(Client *c) {
	...
	if ( (eprop = get_property(c->window, xa_ewm_unmaximised_horz, XA_CARDINAL, &nitems)) ) {
		if (nitems == 2) {
			c->oldx = eprop[0];
			c->oldw = eprop[1];
		}
	...
	if ( (eprop = get_property(c->window, xa_ewm_unmaximised_vert, XA_CARDINAL, &nitems)) ) {
		if (nitems == 2) {
			c->oldy = eprop[0];
			c->oldh = eprop[1];
		}
...
static void reparent(Client *c) {
	...
	grab_button(c->parent, Mod1Mask | altmask, AnyButton);
}
